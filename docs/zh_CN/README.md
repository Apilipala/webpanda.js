...
工程之间不要相互继承，这样会造成程序终止执行。这是因为在执行派生工程时，会先准备被继承的工程文件，然后要等待被继承的工程准备完毕才执行派生工程。如果派生工程与被继承工程相互继承，这就出现了死循环，而为了防止程序死循环框架将会终止这样的执行流程。
...
工程 “home” 出现重复准备，检查工程定义是否存在多个工程之间互相继承，为了防止死循环，当前进度已被终止。
